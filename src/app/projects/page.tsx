      ...
      >
        {projects.map(project => (
          <Card as={'li'} key={project.name}>
            ...
            >
              <Image src={project.logo} alt={'Logo'} className={'h-8 w-8'} unoptimized />
            </div>

            <h2 className={'mt-6 text-base font-semibold text-zinc-800 dark:text-zinc-100'}>
              <Card.Link href={project.link.href} className={''}>
                {project.name}
              </Card.Link>
            </h2>
            ...
            <p
              className={
                'relative z-10 mt-6 flex items-center text-sm font-medium text-zinc-400 transition group-hover:text-teal-500 dark:text-zinc-100'
              }
            >
