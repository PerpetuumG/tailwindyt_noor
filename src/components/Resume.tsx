...
const Role = ({ role }: { role: Role }) => {
  ...
  return (
    <li className={`flex gap-4`}>
      <div
        className={`relative mt-1 flex w-10 h-10 flex-none items-center justify-center 
        rounded-full shadow-md shadow-zinc-800/5 ring-1 ring-red-900/5 dark:border 
        dark:border-zinc-700/50 dark:bg-zinc-800 dark:ring-0`}
      >
        <Image src={role?.logo} alt={'Logo'} className={'h-7 w-7'} unoptimized />
      </div>
